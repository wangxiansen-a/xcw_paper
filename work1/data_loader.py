...
class MinMaxNormalizer:
    ...
    def fit(self, data):
        ...
        self.range_val = np.where(self.range_val == 0, 1.0, self.range_val)  # 防止除零
...
def load_raw_data():
    ...
    """
    # 加载并转置数据
    params = pd.read_csv(PARAMS_FILE, header=None).values[:INPUT_DIM, :].T   # (N, 3)
...
def create_forward_dataloaders(train_data, test_data, batch_size=32):
    """
    创建前向网络的数据加载器
    输入: 结构参数
    输出: 吸收光谱
...
def create_tandem_dataloaders(train_data, test_data, batch_size=32):
    """
    创建串联网络的数据加载器
    输入和目标都是光谱（逆向设计场景）
    """
